...
def register_medico():
    ...
    if form.validate_on_submit():
        ...
        if existing_user:
            flash('Ya existe un usuario registrado con ese mail', 'error')
            form.email.data = ''
            return render_template('/registros/registrar_medico.html', form=form, password=request.form['password'])
        ...
        # Asignamos el rol de "Médico", por ejemplo id_rol=2
        new_medico = Usuario(
            nombre=form.nombre.data,
            apellido=form.apellido.data,
            email=form.email.data,
            ...
            fecha_nacimiento=form.fecha_nacimiento.data,
            telefono=form.telefono.data,
            # matricula=form.matricula.data,   # Nuevo campo de matrícula
            # especialidad=form.especialidad.data,  # Nuevo campo de especialidad
            id_rol=2,  # Asignamos el rol específico para médicos
        )
